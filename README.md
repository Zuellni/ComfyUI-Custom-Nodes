...
IF&nbsp;Encode | Encodes prompts for use with `IF Stage I` and `IF Stage II`. Setting `unload` to `True` removes the model from memory. `Device` functions as above. Prompts can be reused without having to reload the encoder.
